<template>
  <div>
    ...
    <div v-if="!canReadServiceTiers" class="min-h-screen bg-gray-50 py-8">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-xl rounded-lg p-8 text-center">
          ...
          <p class="mt-2 text-sm text-gray-500">
            You don't have permission to access service tier management.
    ...
    <div v-else>
        ...
        <div class="p-6">
          <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex gap-2">
              ...
              <!-- No Create Permission Indicator -->
              <div v-else-if="!canCreateServiceTiers" class="flex items-center">
              ...
              <tbody class="table-body">
                ...
                <template v-else v-for="tier in filteredServiceTiers" :key="tier.service_tier_id">
                  ...
                  <tr class="table-row">
                    ...
                    <td class="table-cell">
                      <div class="flex items-center gap-2">
                        <!-- View Button - always available if user can read -->
                        <button
      ...
      <div v-if="showCreateModal && canCreateServiceTiers" class="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div v-if="showEditModal && canEditServiceTiers" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
      ...
      <div v-if="showDeleteModal && canDeleteServiceTiers" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
...
const canReadServiceTiers = computed(() => {
  const userPermissions = authStore.user?.role?.permissions?.service_tiers || []
  return userPermissions.includes('read')
})

const canCreateServiceTiers = computed(() => {
  const userPermissions = authStore.user?.role?.permissions?.service_tiers || []
  return userPermissions.includes('create')
})

const canEditServiceTiers = computed(() => {
  const userPermissions = authStore.user?.role?.permissions?.service_tiers || []
  return userPermissions.includes('update')
})

const canDeleteServiceTiers = computed(() => {
  const userPermissions = authStore.user?.role?.permissions?.service_tiers || []
...
const loadServiceTiers = async () => {
  if (!canReadServiceTiers.value) {
    error.value = 'Permission denied. You do not have permission to view service tiers.'
    return
  }
...
const handleCreateServiceTier = async () => {
  if (!canCreateServiceTiers.value) {
    createError.value = 'Permission denied. You do not have permission to create service tiers.'
    return
  }
...
const editServiceTier = (tier) => {
  if (!canEditServiceTiers.value) {
    alert('Permission denied. You do not have permission to edit service tiers.')
    return
  }
...
const handleUpdateServiceTier = async () => {
  if (!canEditServiceTiers.value) {
    editError.value = 'Permission denied. You do not have permission to update service tiers.'
    return
  }
...
const deleteServiceTier = (tier) => {
  if (!canDeleteServiceTiers.value) {
    alert('Permission denied. You do not have permission to delete service tiers.')
    return
  }
...
onMounted(async () => {
  ...
  if (viewId && canReadServiceTiers.value) {
